...
public class AlertasServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (!DashboardService.puedeVerPanel(usuario, "alertas")) {
            resp.sendError(HttpServletResponse.SC_FORBIDDEN, "Sin acceso a alertas");
            return;
        }
        ...
        resp.setHeader("X-Modulo", "Alertas");
        req.getRequestDispatcher("/alertas.jsp").forward(req, resp);
    }
    ...
    private Usuario validarSesion(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if (DashboardService.esVendedor(usuario)) {
            resp.sendRedirect("alertas-venta");
            return null;
        }
        return usuario;
    }
